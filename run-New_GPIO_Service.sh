...
then
	...
	then
		...
		$a "1/2 - Servicio, crear archivo .service de $nomServ, hecho."
		...
		$a "2/2 - Servicio, habilitar, recargar, reiniciarlo, hecho."
...
then
	...
	then
		...
		eliminar_servicio=$(ls /etc/systemd/system/ | grep "servicio.service" | grep $eliminar_pin)
			if [ $?  = 0 ];
			then
				$a "Parar servicio $eliminiar_servicio"
				systemctl stop $eliminar_servicio
				$a
				$a "Deshabilitar servicio $eliminar_servicio"
				systemctl disable $eliminar_servicio
				$a
				$a "Quitar servicio $eliminar_servicio del sistema"
